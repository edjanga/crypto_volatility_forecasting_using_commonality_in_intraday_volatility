...
class Reader:
    ...
    def returns_read(self, cutoff_low: float = .01, cutoff_high: float = .01, raw: bool=False) -> pd.DataFrame:
    ...
    def rv_read(self, cutoff_low: float = .01, cutoff_high: float = .01, raw: bool=False) -> pd.DataFrame:
        rv_df = self.returns_read(cutoff_low=cutoff_low, cutoff_high=cutoff_high, raw=raw)**2
