...
class EDA:
    ...
    def optimal_clusters(self, save: int) -> None:
        ...
        fig.update_layout(title=dict(text='Optimal number of clusters: Analysis', font=dict(size=TITLE_FONT_SIZE)),
    ...
    def daily_rv(self, save: int) -> None:
        ...
        fig.update_layout(title=dict(text='Annualized daily RV', font=dict(size=TITLE_FONT_SIZE)),
                          width=WIDTH, height=HEIGHT, font=dict(size=LABEL_AXIS_FONT_SIZE))
        fig.update_annotations(font=dict(size=TITLE_FONT_SIZE), x=1)
        if save == 1:
    ...
    def intraday_rv(self, save: int) -> None:
        ...
        fig.update_yaxes(title_text='RV') #height=500, width=800
        fig.update_layout(title=dict(text='Annualized intraday RV: Statistics', font=dict(size=TITLE_FONT_SIZE)),
                          width=WIDTH, height=HEIGHT, font=dict(size=LABEL_AXIS_FONT_SIZE))
        if save == 1:
    ...
    def daily_mean_correlation_matrix(self, save: int) -> None:
        corr = self.rv.copy()
        ...
        fig.update_layout(title=dict(text='Daily pairwise RV correlation mean', font=dict(size=TITLE_FONT_SIZE)),
                          width=WIDTH, height=HEIGHT, font=dict(size=LABEL_AXIS_FONT_SIZE))
        if save == 1:
    ...
    def daily_pairwise_correlation(self, save: int) -> None:
    ...
    def boxplot(self, save: int, transformation: str = None):
...
class PlotResults:
    ...
    _models_ls = ['risk_metrics', 'ar', 'har', 'har_eq']
