...
if __name__ == '__main__':
    ...
    if isinstance(args.models, str):
        ...
        models = args.models
    if len(set(models).intersection(set(['risk_metrics']))) > 0:
    ...
    to_csv_dd = {True: 'w', False: 'a'} #Does not exist?
    model_builder_obj.reinitialise_models_forecast_dd()
    print_tag = (model_builder_obj.model_type,
                 model_builder_obj.L, model_builder_obj.F[0], cross_name_dd[args.cross],
                ...
                cross_name_dd[args.cross], transformation_dd[args.transformation], regression_type)
    print(f'[Computation]: Compute all tables for {print_tag}...')
    """
    Generate all tables for model L, F cross
    """
    if model_builder_obj.model_type in ['har', 'har_dummy_markets', 'har_universal', 'ar']:
        model_builder_obj.add_metrics(df=rv, cross=args.cross, agg=agg, transformation=args.transformation,
                                      variance_explained=var_explained, regression_type=regression_type)
    elif model_builder_obj.model_type == 'har_cdr':
        model_builder_obj.add_metrics(df=rv, df2=cdr, cross=args.cross, agg=agg, transformation=args.transformation,
                                      variance_explained=var_explained, regression_type=regression_type)
    elif model_builder_obj.model_type == 'har_csr':
        model_builder_obj.add_metrics(df=rv, df2=csr, cross=args.cross, agg=agg, transformation=args.transformation,
                                      variance_explained=var_explained, regression_type=regression_type)
    elif model_builder_obj.model_type in ['risk_metrics']:
        model_builder_obj.add_metrics(df=returns, agg=agg, transformation=args.transformation,
    ...
    if args.cross & (model_builder_obj._model_type != 'har_universal'):
        ...
        if (not args.cross) & (model_builder_obj.model_type not in ['ar', 'risk_metrics']):
            coefficient = coefficient.T[['const']+model_builder_obj.F+model_specific_features].T
        elif model_builder_obj.model_type != 'risk_metrics':
    ...
    y = pd.concat(model_builder_obj.models_forecast_dd).droplevel(0)
    y.index.name = 'timestamp'
    """
    Table insertion for y and coefficients
    """
    ...
    print(
        f'[Creation]: '
        f'y and coefficient for {table_name_suffix_ls} - {model_builder_obj.model_type} have been generated.')
    ...
    if test:
        r2.to_csv(f'{"_".join(["r2"]+table_name_suffix_ls)}.csv', mode='a',
                  header=not os.path.exists(f'{"_".join(["r2"]+table_name_suffix_ls)}.csv'),
                  index=not os.path.exists(f'{"_".join(["r2"]+table_name_suffix_ls)}.csv'))
        mse.to_csv(f'{"_".join(["mse"]+table_name_suffix_ls)}.csv', mode='a',
                   header=not os.path.exists(f'{"_".join(["mse"]+table_name_suffix_ls)}.csv'),
                   index=not os.path.exists(f'{"_".join(["r2"]+table_name_suffix_ls)}.csv'))
        qlike.to_csv(f'{"_".join(["qlike"]+table_name_suffix_ls)}.csv', mode='a',
                     header=not os.path.exists(f'{"_".join(["qlike"]+table_name_suffix_ls)}.csv'),
                     index=not os.path.exists(f'{"_".join(["r2"]+table_name_suffix_ls)}.csv'))
        print(f'[Creation]: All rolling metrics tables for {print_tag} have been generated.')
    else:
        ...
        qlike.to_sql(con=model_builder_obj.db_connect_qlike, name=f'{"_".join(["qlike"]+table_name_suffix_ls)}',
                     if_exists='append')
        print(f'[Insertion]: All rolling metrics tables for {print_tag} have been inserted into the database.')
    """
        Average number of PCAs per model
    """
    if args.cross & (model_builder_obj.model_type != 'har_universal'):
        pca_number = pd.DataFrame(model_builder_obj.pca_components_models_dd)
        pca_number = pd.concat([pca_number[col].apply(lambda x:x[0]) for col in pca_number.columns], axis=1)
        pca_number.index.name = 'symbol'
        #pca_number = pca_number.mean()
        #pca_number.name, pca_number.index.name = 'values', 'model'
        #pca_number = pd.DataFrame(pca_number).reset_index()
        pca_number.to_csv(os.path.abspath(f'./model/PCA_{args.L}.csv'),
                          mode=to_csv_dd[os.path.exists(f'./model/PCA_{args.L}.csv')],
                          index=os.path.exists(f'./model/PCA_{args.L}.csv'),
                          header=os.path.exists(f'./model/PCA_{args.L}.csv')) if test \
            else pca_number.to_sql(con=model_builder_obj.db_connect_pca, name=f'PCA_{args.L}', if_exists='append')
    ...
    outliers.to_csv(f'outliers_{args.L}.csv') if test else \
        outliers.to_sql(con=model_builder_obj.db_connect_outliers, name=f'outliers_{args.L}',
                        if_exists='replace')
    if not test:
        """
        Close databases
        ...
        model_builder_obj.db_connect_pca.close()